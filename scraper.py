"""
scraper.py — Scrapes ZenMarket for archive fashion listings.

ZenMarket aggregates Mercari JP and Yahoo Auctions Japan in English.
Search URL pattern: https://zenmarket.jp/en/search.aspx?q=KEYWORD&cat=0
"""
...
def _fallback_extract(soup: BeautifulSoup) -> list:
    """
    Last-resort extraction: find all <a> tags linking to auction pages
    and treat their parent containers as cards.
    """
    seen = set()
    cards = []
    for a in soup.select("a[href*='/en/auction/'], a[href*='zenmarket.jp/en/']"):
        parent = a.parent
